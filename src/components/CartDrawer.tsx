        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-5 py-16">
              <IconShoppingCart className="w-16 h-16 text-[#E85D04]" aria-hidden />
              <div>
              ...
              <button
                onClick={closeCart}
                className="px-6 py-2.5 bg-[#E85D04] text-white text-xs font-heading font-bold tracking-widest uppercase hover:bg-[#C44A00] transition rounded"
              >
                Continue Shopping
              </button>
            </div>
              ...
              return (
                <div key={item.product_code} className="flex gap-4 p-3 bg-[#1A1A1A] border border-[#2B2B2B] rounded">
                  ...
                  <div className="relative w-20 h-20 flex-shrink-0 bg-[#2B2B2B] rounded overflow-hidden">
                    {imgUrl ? (
                      <Image src={imgUrl} alt={item.product_name} fill className="object-contain p-1.5" sizes="80px" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      onClick={closeCart}
                      className="text-sm font-heading font-bold text-white uppercase tracking-wide line-clamp-2 hover:text-[#E85D04] transition leading-snug"
                    >
                      {item.product_name}
                    </Link>
                    {item.product_sku && (
