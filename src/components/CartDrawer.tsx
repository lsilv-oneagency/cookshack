      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2B2B2B]">
          <div className="flex items-center gap-3">
            ...
            {itemCount > 0 && (
              <span className="bg-[#AE1B07] text-white text-xs font-bold px-2 py-0.5 rounded-full">
                {itemCount}
              </span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-5 py-16">
              <IconShoppingCart className="w-16 h-16 text-[#AE1B07]" aria-hidden />
              <div>
              ...
              <button
                onClick={closeCart}
                className="px-6 py-2.5 bg-[#AE1B07] text-white text-xs font-heading font-bold tracking-widest uppercase hover:bg-[#8E1405] transition rounded"
              >
                Continue Shopping
              </button>
            </div>
                ...
                <div key={item.product_code} className="flex gap-4 p-3 bg-[#1A1A1A] border border-[#2B2B2B] rounded">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      onClick={closeCart}
                      className="text-sm font-heading font-bold text-white uppercase tracking-wide line-clamp-2 hover:text-[#AE1B07] transition leading-snug"
                    >
                      {item.product_name}
                    </Link>
                    {item.product_sku && (
