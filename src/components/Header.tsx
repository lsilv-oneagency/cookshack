        ...
        <div className="glass-header-layer border-b border-white/[0.18] text-[#9A9A9A] text-xs">
          <div className="mx-auto flex h-10 max-w-7xl items-center justify-between px-4 sm:h-9 sm:px-6 lg:px-8">
            <span className="flex min-w-0 items-center gap-1.5">
              <svg className="h-3.5 w-3.5 shrink-0 text-[#AE1B07]" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="flex shrink-0 items-center gap-2 sm:gap-4 md:gap-5">
              ...
              >
                ...
                {itemCount > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-[#AE1B07] px-0.5 text-[9px] font-bold text-white sm:h-[18px] sm:min-w-[18px] sm:text-[10px]">
          ...
          <div className="relative flex min-h-[5rem] items-center justify-center py-2.5 sm:min-h-[5.25rem] sm:py-3">
            ...
            <Link href="/" className="group px-2" aria-label="Cookshack — Home">
              <CookshackLogoLockup
                ...
                taglineClassName="font-heading font-bold uppercase leading-none text-[0.52rem] tracking-[0.22em] text-[#AE1B07]"
              />
            </Link>
          </div>
            ...
            <form onSubmit={handleSearch} className="mx-auto w-full max-w-3xl">
              <div className="relative w-full">
                <input
                  ...
                  className="w-full rounded-full border border-white/20 bg-white/[0.14] py-2.5 pl-4 pr-11 text-sm text-white shadow-inner shadow-black/30 placeholder:text-white/50 backdrop-blur-xl backdrop-saturate-200 transition focus:border-white/35 focus:bg-white/[0.2] focus:outline-none focus:ring-2 focus:ring-[#AE1B07]"
                />
                <button
                  type="submit"
                  title="Search"
                  aria-label="Search"
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#6B6B6B] transition hover:text-[#AE1B07]"
                >
                    ...
                    <Link
                      ...
                      className={`flex min-h-[40px] w-full min-w-0 items-center justify-center gap-0.5 px-1 py-2 text-center text-[10px] font-heading font-semibold uppercase leading-none tracking-wide transition border-b-2 sm:min-h-[44px] sm:px-1.5 sm:text-[11px] lg:px-2 lg:text-xs ${
                        megaOpen || pathname.startsWith(item.href)
                          ? "text-[#AE1B07] border-[#AE1B07]"
                          : "text-[#CCCCCC] hover:text-white border-transparent hover:border-[#AE1B07]"
                      }`}
                    >
              ...
              <div className="mx-auto max-w-7xl px-3 py-3 sm:px-4 lg:px-6">
                <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Link
                    ...
                    className="text-[10px] font-heading font-bold uppercase tracking-widest text-[#AE1B07] hover:text-[#8E1405] transition sm:text-xs"
                  >
                ...
                {megaLoading ? (
                  ...
                  <p className="py-4 text-center text-xs text-[#9A9A9A] sm:text-sm">
                    ...
                    <Link href={activeMegaItem.href} className="text-[#AE1B07] hover:underline">
                      Browse the category
                    </Link>
                  </p>
                ) : (
                  <div className="grid grid-cols-8 gap-1.5 sm:gap-2">
                    {megaProducts.map((p) => (
                      <Link
                        ...
                        className="group flex min-w-0 flex-col overflow-hidden rounded-lg border border-white/15 bg-white/95 shadow-sm backdrop-blur-md transition hover:border-[#AE1B07] hover:bg-white hover:shadow-md"
                      >
                        ...
                        <div className="flex min-h-0 flex-1 flex-col gap-0.5 px-1 pb-1.5 pt-1">
                          <span className="line-clamp-2 text-[8px] font-heading font-semibold uppercase leading-tight tracking-wide text-neutral-900 transition group-hover:text-[#AE1B07] sm:text-[9px] md:line-clamp-2 md:text-[10px]">
                            {p.name}
                          </span>
                          {p.formatted_price && (
                            <span className="text-[9px] font-heading font-extrabold text-[#AE1B07] sm:text-[10px] md:text-xs">
                              {p.formatted_price}
                            </span>
                          )}
                        </div>
            ...
            <nav className="py-2">
              ...
              <div className="border-t border-[#2B2B2B] mt-2 pt-2 px-5 pb-3 flex gap-4 text-xs text-[#9A9A9A]">
                <a href="tel:18004230698" className="inline-flex items-center gap-1.5 hover:text-white">
                  <IconPhone className="w-3.5 h-3.5 shrink-0 text-[#AE1B07]" aria-hidden />
                  1-800-423-0698
                </a>
